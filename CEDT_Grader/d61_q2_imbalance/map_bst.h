...
class map_bst
{
  ...
  public:
    ...
    int get_height(node *n,KeyT &keydude,int &maxHeight){
      if(!n) return -1;


      int a = get_height(n->left,keydude,maxHeight);
      int b = get_height(n->right,keydude,maxHeight);


      int diff_height = abs(a-b);

      if(diff_height > maxHeight){
        keydude = n->data.first;
        maxHeight = diff_height;
      }
      else if(diff_height == maxHeight){
        if(keydude > n->data.first){
          keydude = n->data.first;
        }
      }
    ...
    KeyT getValueOfMostImbalanceNode() {
      ...
      KeyT keys = mRoot->data.first;
      int maxHeight = -1;

      get_height(mRoot,keys,maxHeight);
